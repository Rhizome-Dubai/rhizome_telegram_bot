...
async def get_user_and_chat(update: Update):
    if (message := update.message) is not None:
        user = message.from_user
        chat = message.chat
    elif (message := update.effective_message) is not None:
        user = message.from_user
        chat = message.chat
    else:
        user = None
    ...
    if user is None or chat is None:
        return False, user, chat
    else:
        return True, user, chat

    

async def register_user_if_not_exists(chat: Chat, context: CallbackContext, user: User):
...
async def message_handle(update: Update, context: CallbackContext, message=None, use_new_dialog_timeout=True):
    ...
    async def message_handle_fn():
        ...
        if use_new_dialog_timeout:
            if (datetime.now() - db.get_user_attribute(user.id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user.id)) > 0:
                db.start_new_dialog(user.id)
        ...
        try:
            ...
            db.update_n_used_tokens(user.id, current_model, n_input_tokens, n_output_tokens)
...
async def voice_message_handle(update: Update, context: CallbackContext):
    ...
    db.set_user_attribute(user.id, "n_transcribed_seconds", voice.duration + db.get_user_attribute(user.id, "n_transcribed_seconds"))
...
async def generate_image_handle(update: Update, context: CallbackContext, message=None):
    ...
    # token usage
    db.set_user_attribute(user.id, "n_generated_images", config.return_n_generated_images + db.get_user_attribute(user.id, "n_generated_images"))
...
async def set_settings_handle(update: Update, context: CallbackContext):
    ...
    db.set_user_attribute(user.id, "current_model", model_key)
    db.start_new_dialog(user.id)
...
async def show_balance_handle(update: Update, context: CallbackContext):
    ...
    n_generated_images = db.get_user_attribute(user.id, "n_generated_images")
    n_transcribed_seconds = db.get_user_attribute(user.id, "n_transcribed_seconds")
